...
<html>
 ...
 <script>
  function demoData(sp){
	...
	if (sp == "badger2"){
		$("#genes").val("m_meles2.gff"); $("#genes_v").val("1.1"); $("#genes_d").val("Another GFF3 file generated by some gene prediction software");
		$("#mrna_trans").val("m_meles2.transcripts.fasta"); $("#mrna_trans_v").val("1.1"); $("#mrna_trans_d").val("A set of transcripts based on the predicted genes");
		$("#mrna_pep").val("m_meles2.proteins.fasta"); $("#mrna_pep_v").val("1.1"); $("#mrna_pep_d").val("A set of proteins based on the predicted genes");
	}
  }
  </script>
	...
	<div class="inline">
		...
		<table width=100%>
			...
			<tr>
				...
				<td width="5%"><br><select name="search_genes">
					<option value="pub">Public</option>
					<option value="priv">Private</option>
				</select></td>
...
<g:if test="${'Genes' in genome.files.file_type}">
	<div class="inline">
